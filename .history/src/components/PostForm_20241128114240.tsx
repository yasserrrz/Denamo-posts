...
const PostForm: React.FC<Props> = ({ setIsModalOpen }) => {
  ...
  return (
    <Form layout="vertical" onFinish={()=>{handleFormSubmit(inputs)}}>
      ...
      >
        <Input onChange={(e) => setInputs({ ...inputs, title: e.target.value })} placeholder="Enter post title" />
      </Form.Item>
      <Form.Item
        name="body"
        label="Body"
      ...
      >
        <Input.TextArea value={} rows={4} placeholder="Enter post body" onChange={(e) => setInputs({ ...inputs, body: e.target.value })} />
      </Form.Item>
      <Form.Item>
